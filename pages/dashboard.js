      ...
      <div className="min-h-screen bg-asphalt-900">
        ...
        <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
                ...
                {bulkResults.length > 0 && (
                  <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
                    <p className="text-xs font-mono text-gray-500">
                      ✓ {bulkResults.filter(r => r.status === 'ok').length} berhasil &nbsp;·&nbsp;
                      ✗ {bulkResults.filter(r => r.status === 'error').length} gagal
                    </p>
                    {bulkResults.map((r, i) => (
          ...
          <div>
            ...
            {loading
              ? <div className="text-center py-10 text-gray-600 font-mono text-sm">Memuat data...</div>
              : filtered.length === 0
