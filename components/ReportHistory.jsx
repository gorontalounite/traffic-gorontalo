        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-gray-500">
              {reports?.length || 0} laporan masuk
            </p>
          </div>
        </div>
        <svg
      ...
      {open && (
        <div className="slide-in-up mt-2 rounded-2xl border border-asphalt-600 bg-asphalt-800 overflow-hidden">
          ...
          <div className="px-4 pt-3 pb-2 border-b border-asphalt-700">
            <div className="flex gap-1.5 overflow-x-auto pb-1">
              <button
                ...
                className={`flex-shrink-0 text-xs rounded-full px-3 py-1 border transition-all font-mono ${
                  filter === 'all'
                    ? 'bg-asphalt-600 border-asphalt-500 text-gray-200'
                    : 'bg-transparent border-asphalt-700 text-gray-500 hover:text-gray-300'
                }`}
              >
                Semua
              </button>
              {KECAMATAN_LIST.map((kec) => (
                <button
                  key={kec.id}
                  onClick={() => setFilter(kec.id)}
                  className={`flex-shrink-0 text-xs rounded-full px-3 py-1 border transition-all font-mono whitespace-nowrap ${
                    filter === kec.id
                      ? 'bg-asphalt-600 border-asphalt-500 text-gray-200'
                      : 'bg-transparent border-asphalt-700 text-gray-500 hover:text-gray-300'
                  }`}
          ...
          <div className="max-h-72 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-10 text-gray-600 text-sm font-mono">
                Memuat laporan...
              </div>
            ) : filtered.length === 0 ? (
                    ...
                    <div key={report.id || i} className="px-4 py-3 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <span className="text-xs font-mono font-500 text-gray-300">
                              {report.kecamatan_nama}
                            </span>
                            <span className={`ml-2 text-xs ${cfg?.text} font-mono`}>
                              · {cfg?.label}
                            </span>
                          </div>
